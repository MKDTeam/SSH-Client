...
Item {
	...
	DropArea {
		...
		Menu {
			...
			MenuItem { 
                highlighted: true
                text: "Найти одинаковые файлы"
				onTriggered: {
					//send_command("find " + data_model.folder + " -type f -maxdepth 1 -perm -o+r -exec md5 {} \\; | sed 's/[ ]//g' | tr = ' ' | sort -t ' ' -k 2 | awk '{sum[$2] += 1; if(sum[$2] == 1) print \"- \" $0; else print \"  \" $0}'")
					//console.log(data_model.folder)
					folder_menu.close()
					MD5_compare(data_model.folder)
				}
			}
		}
        ...
        MouseArea {
            ...
            onClicked: {
                ...
                if(mouse.button & Qt.RightButton) {
                    folder_menu.x = mouseX
                    folder_menu.y = mouseY
                    folder_menu.open()
                }
            }
        }
				...
				Image {
					...
					MouseArea {
						...
						onClicked: {
							...
							if(mouse.button & Qt.RightButton) {
								file_menu.x = dragArea.mouseX
								file_menu.y = dragArea.mouseY
								file_menu.open()
							}
						}
